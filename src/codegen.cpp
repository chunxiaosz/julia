...
{
    Function *F = cast<Function>(function);
...
static jl_cgval_t emit_getfield(jl_codectx_t &ctx, const jl_cgval_t &strct, jl_sym_t *name)
{
    ...
    jl_cgval_t ret = mark_julia_type(ctx, result, true, jl_any_type, needsgcroot);
    return ret;
}
...
//        }
        ...
        else if (jl_is_leaf_type(obj.typ)) {
            Value *sz;
            if (obj.constant) {
        ...
        const jl_cgval_t &fld = argv[2];
        if (jl_is_type_type(typ.typ) || typ.typ == (jl_value_t*)jl_datatype_type) { // FIXME: this predicate is inaccurate (incorrect)
            if (fld.typ == (jl_value_t*)jl_long_type) {
                assert(typ.isboxed);
                ...
                Value *idx = emit_unbox(ctx, T_size, fld, (jl_value_t*)jl_long_type);
                emit_bounds_check(ctx, typ.typ, (jl_value_t*)jl_datatype_type, idx, types_len);
...
static jl_cgval_t emit_local(jl_codectx_t &ctx, jl_value_t *slotload)
{
    ...
    assert((int)sl != ctx.vaSlot && "error in escape analysis");
    jl_varinfo_t &vi = ctx.slots[sl];
                                  ...
                                  jl_typemap_entry_t *sf, jl_value_t *declrt, jl_tupletype_t *sigt)
{
    ...
    if (lam && lam->jlcall_api == 2) {
        ...
        if (lam) {
            ...
            if (fname) {
                theFptr = cast<Function>(jl_Module->getNamedValue(fname));
                if (!theFptr) {
...
static jl_returninfo_t get_specsig_function(Module *M, const std::string &name, jl_value_t *sig, jl_value_t *jlrettype)
{
    ...
    Function *f = M ? cast<Function>(M->getNamedValue(name)) : NULL;
    if (f == NULL) {
